...
import com.ashish.cards.dto.CardsContactInfoDto;
...
public class CardsController {
    ...
    @GetMapping("/fetch")
    public ResponseEntity<CardsDto> fetchAccountDetails(@RequestHeader("eazybank-correlation-id") String correlationId,
                                                        @RequestParam
                                                           @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits")
                                                           String mobileNumber) {

        logger.debug("fetchCardDetails method start");
        CardsDto customerDto = cardService.fetchCard(mobileNumber);
    ...
    @PutMapping("/update")
    public ResponseEntity<ResponseDto> updateCard(@Valid@RequestBody CardsDto dto) {
        boolean isUpdated = cardService.updateCard(dto);
    ...
    @Operation(
            ...
            description = "REST API to get env information"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
